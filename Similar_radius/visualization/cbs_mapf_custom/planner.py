...
# The low level planner for CBS is A*
from stastar_custom.planner import Planner as STPlanner
...
class Planner:
    ...
    def plan(self, starts: List[Tuple[int, int]],
                   goals: List[Tuple[int, int]], ## preference  before assign
                   assign:Callable = min_cost4,
                   max_iter:int = 200,
        ...
        # Do goal assignment
        self.agents = assign(starts, goals) ## optional preference argument

        constraints = Constraints()

        # Compute path for each agent using low ltevel planner
    ...
    def search_node(self, best: CTNode, results):
        ...
        #print conflict and constraints
        #print(f"Conflicts between agent {agent_i.id} and agent {agent_j.id} at time{time_of_conflict}")
        #print(f"Constraints on agent {agent_i.id}:{best.constraints[agent_i]}")
        #print(f"Constraints on agent {agent_j.id}:{best.constraints[agent_j]}")
        #print("hello")

        #conflicts.append((agent_i, agent_j, time_of_conflict))
        #constraints_list.append((agent_i_constraint, agent_j_constraint))
        #print("hello")
        ...
        #for idx, (conflict,constraints) in enumerate (zip(conflicts, constraints_list)):
        #    print(f"Conflicts {idx+1}: Agent {conflict[0]} and Agent{conflict[1]} at time{conflict[2]}")
        #   print (f"Constraints: {constraints[0]} and {constraints[1]}")
