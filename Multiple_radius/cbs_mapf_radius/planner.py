...
class Planner:
    ...
    def plan(self, starts: List[Tuple[int, int]],
        ...
        # Do goal assignment
        self.agents = assign(starts, goals, self.robot_radius)  
    ...
    def safe_distance(self, solution: Dict[Agent, np.ndarray], agent_i: Agent, agent_j: Agent) -> int:
        for idx, (point_i, point_j) in enumerate(zip(solution[agent_i], solution[agent_j])):
            ...
            if self.dist(point_i,point_j, agent_i.radius, agent_j.radius) > agent_i.radius + agent_j.radius:     
                continue
            return idx
        return -1

    @staticmethod
    def dist(point1: np.ndarray, point2: np.ndarray, radius1 : int , radius2: int ) -> int: 
        return int(np.linalg.norm(point1-point2, 2)) + radius1 + radius2 # L2 norm    
    ...
    def calculate_constraints(self, node: CTNode,
        ...
        try:
           # while self.dist(contrained_path[conflict_end_time], pivot) < 2*self.robot_radius:
             while self.dist(contrained_path[conflict_end_time], pivot) < (constrained_agent.radius + unchanged_agent.radius): 
                conflict_end_time += 1
        except IndexError:
            pass
