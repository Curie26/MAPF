...
class Planner:
    ...
    def __init__(self, grid_size: int,
                       robot_radius: List[int], ## edit this to take a list 
    ...
    '''
    def safe_static(self, grid_pos: np.ndarray, radius: int) -> bool: ### edited this added radius : int
        _, nn = self.static_obstacles.query(grid_pos)
        for i in radius:
            if self.l2(grid_pos, self.static_obstacles.data[nn]) <= i:
                return False
        return True
    ...
    def plan(self, start: Tuple[int, int],
                   ...
                   max_iter:int = 500,
                   radius: int = None, ##### added this line 
        ...
        def safe_dynamic(grid_pos: np.ndarray, time: int, radius: int) -> bool: ## added radius : int
            nonlocal dynamic_obstacles
            return all(self.l2(grid_pos, obstacle) > 2 * radius  ## changed self.robot_radius to radius
        ...
        def safe_semi_dynamic(grid_pos: np.ndarray, time: int, radius: int) -> bool: ## added radius : int
            nonlocal semi_dynamic_obstacles
            for timestamp, obstacles in semi_dynamic_obstacles.items():
                flag = True
                if time >= timestamp:
                    flag = all(self.l2(grid_pos, obstacle) > 2 * radius for obstacle in obstacles) ## changed self.robot_radius to radius
                if not flag:
                    return False
            return True
        ...
        radius = self.robot_radius ## added this line
        ...
        while open_set and iter_ < max_iter:
            ...
            for neighbour in self.neighbour_table.lookup(current_state.pos):
                ...
                # Avoid obstacles #### added radius parameter for all these 
                if not self.safe_static(neighbour,radius) \
                   or not safe_dynamic(neighbour, epoch, radius) \
                   or not safe_semi_dynamic(neighbour, epoch, radius):
                    continue
